...
contract TutorialToken is ERC20 {
  ...
  function mint(uint256 amount) external {
    ...
    balances[_owner] = safeAdd(balances[_owner], amount);
  ...
  function burn(uint256 amount) external {
    ...
    balances[_owner] = safeSubtract(balances[_owner], amount);
    _totalSupply = safeSubtract(_totalSupply, amount);
    emit Burn(amount);
  }  
  ...
  function _transfer(address sender, address recipient, uint amount ) private {
    ...
    balances[sender] = safeSubtract(balances[sender], amount);
